...
public class YamlItem {
    ...
    public YamlItem(final String path) {
        this.path = path;
        this.setType(((DailyTaskMain.config.get(path + ".type") instanceof String)) ? Material.valueOf(DailyTaskMain.config.getString(path + ".type").toUpperCase()) : Material.AIR);
        this.setName(((DailyTaskMain.config.get(path + ".name") instanceof String)) ? DailyTaskMain.config.getString(path + ".name") : null);
        this.setLore(DailyTaskMain.config.getStringList(path + ".lore"));
        this.setSlot(((DailyTaskMain.config.get(path + ".slot") instanceof Integer)) ? DailyTaskMain.config.getInt(path + ".slot") : 0);
        this.setAmount(((DailyTaskMain.config.get(path + ".amount") instanceof Integer)) ? DailyTaskMain.config.getInt(path + ".amount") : 1);
        this.setGlow((DailyTaskMain.config.get(path + ".glow") instanceof Boolean) ? DailyTaskMain.config.getBoolean(path + ".glow") : false);
        this.setHeadValue(((DailyTaskMain.config.get(path + ".head-value") instanceof String)) ? DailyTaskMain.config.getString(path + ".head-value") : null);
    }
    ...
    public String getKey(String key) {
        return DailyTaskMain.config.getString(this.path + "."+key);
    }
    ...
    public YamlItem renewLore() {
        this.setLore(DailyTaskMain.config.getStringList(this.path + ".lore"));
        return this;
    }
    ...
    public ItemStack complete(){
        ...
        if(this.headValue == null){
            ...
            if(((DailyTaskMain.config.get(path + ".model"))) instanceof Integer){
                net.minecraft.server.v1_16_R3.ItemStack nmsitem = CraftItemStack.asNMSCopy(item);
                NBTTagCompound nbtset = new NBTTagCompound();
                if (nmsitem.hasTag()) {
                    nbtset.a(nmsitem.getTag());
                }
                nbtset.setInt("CustomModelData", DailyTaskMain.config.getInt(path + ".model"));
